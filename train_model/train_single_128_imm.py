...
REPORT_PATH = BASE_DIR / "single_128_imm_classification_report.txt"
RANDOM_STATE = 42
N_SPLITS = 5
POSITIVE_LABEL = 1
LABEL_MAP = {"F": 0, "T": 1}
...
IMMEDIATE_UNAVAILABLE_COLUMNS = [
    ...
    "subcomDepth",
    "firstCommentTimeDiff",
    "first30MinCommentRatio",
]
...
def main() -> None:
    ...
    immediate_drop_columns = [
        column for column in IMMEDIATE_UNAVAILABLE_COLUMNS if column in x.columns
    ]
    missing_immediate_columns = [
        column for column in IMMEDIATE_UNAVAILABLE_COLUMNS if column not in x.columns
    ]
    x = x.drop(columns=immediate_drop_columns)
    ...
    print(f"Immediate unavailable features removed: {len(immediate_drop_columns)}")
    if immediate_drop_columns:
        print("Removed features: " + ", ".join(immediate_drop_columns))
    if missing_immediate_columns:
        print("Unavailable feature names not found: " + ", ".join(missing_immediate_columns))
    ...
    print("Imbalance handling: class weight / scale_pos_weight")
